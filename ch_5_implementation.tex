...
This chapter presents the implementation of the system, describing the functional modules, core algorithms, user interfaces, and deployment configurations.
...
This section describes the software frameworks and hardware configurations used during development and deployment.
...
This section describes the implementation details of each major module.

\subsection{Module 1: Registration}

\textbf{Purpose:}
To register new users within the platform.

\textbf{Functionality:}
The portal presents a registration form requiring a name, email address, and password. Client-side validation provides real-time feedback, ensuring the name exceeds three characters, the password contains at least eight characters, and the email format is valid.

\textbf{Important Functions:}

\textbf{SignUp:}
Upon form submission, a server-side action parses the input payload. Invalid submissions are immediately rejected to prevent malicious calls. Upon successful validation, the system registers the account using the provided credentials. A secure session token is subsequently stored in the browser, valid for seven days. This token is automatically refreshed upon active use within the validity window; otherwise, the session expires and re-authentication is required.

\textbf{SendEmail:}
In a development environment, the system logs the verification link to the console for debugging. In a production environment, it dispatches an automated verification email. Following the link immediately authenticates the user. This link has a strict five-minute expiration window; expired links redirect the user to request a fresh verification token.

\subsection{Module 2: Login}

\textbf{Purpose:}
To authenticate existing users into the system.

\textbf{Functionality:}
The system first verifies the presence of an active session token. If a valid token is found, the user is automatically redirected to the dashboard, bypassing the login interface. The login form provides real-time validation for email formatting and password minimum length constraints. It returns standardized error messages for incorrect credentials to prevent credential enumeration.

\textbf{Important Functions:}

\textbf{Email Verification:}
The authentication flow verifies the account's email status. If the email remains unverified, the system prompts the user to complete verification before granting access.

\subsection{Module 3: Student Request Leave}
\textbf{Purpose:}
Allows students to formally submit LRs for specific lectures. These requests are subsequently routed to the BA (BA) and HOD (HOD) for review.
...
\begin{itemize}
\item The target subject (restricted to currently registered courses)
\item The specific lecture date
\item A detailed reason or description
\item An optional supporting document (image format, capped at 5 MB)
\end{itemize}
\item Real-time client-side validation enforces constraints during data entry:
\begin{itemize}
\item Duplicate check: verifies whether a LR already exists for the identical subject and date
\item Subject registration verification: ensures the selected course matches the student's active enrollment profile
\end{itemize}
\item If a duplicate is detected, an immediate error message is displayed.
\item Upon form submission, a server-side action re-validates the constraints:
  \begin{itemize}
  \item The subject must be offered in the current semester
  \item The student must be formally registered in the subject
  \item There must be no pre-existing pending or approved LR for the same session
  \end{itemize}
\item Failed validations trigger a rejection and present localized error messages to the client. 
\item Upon success, the system instantiates a new LR record with a \texttt{PENDING} status, assigning it to the corresponding BA.
\end{itemize}
...
\begin{itemize}
    ...
    \item server action validates inputs, checks permission if student is allowed to create LR, resolves subjectId and offering and student registeration, duplicate check and then creates LR. If fails it shows error to the student.
    \item LR form that fetches student's enrolled subjects and renders LR form.
...
\begin{itemize}
    ...
    \item \textbf{Auth (Better Auth)} requireSession() ensures only authenticated users can create LRs.
    \item \textbf{Toast:} Success/error feedback on form submission
\end{itemize}
Enrollment/subjects.

\subsection{Module 4: Student Announcements}
\textbf{Purpose:}
Displays targeted administrative and departmental announcements to students, using contextual filtering based on their specific program and department.

\textbf{Functionality:}
\begin{itemize}
    \item \textbf{Filtered Feed:} Restricts the announcement feed to match the student's departmental affiliation, alongside system-wide broadcasts.
    \item \textbf{Pin Support:} Prioritizes pinned announcements at the top of the feed, sorting the remainder chronologically.
    \item \textbf{Details View:} Dedicated interface rendering the complete announcement payload, including attached media, timestamps, and authorship.
    \item \textbf{Responsive Sidebar:} A contextual right sidebar surfaces a preview feed on desktop layouts.
    \item \textbf{Asynchronous Loading:} Implements React Suspense boundaries and skeleton loaders to stream list and detail views smoothly.
\end{itemize}

\textbf{Important Functions:}

\begin{itemize}
    \item \textbf{studentGetAllAnnouncements:} Resolves the student's active department from their registration record, constructs a scoped Prisma query clause, and applies the sorting logic for pinned and recent items.
    \item \textbf{studentGetAnnouncementById:} Retrieves the full payload of a discrete announcement for the dedicated detail view.
    \item \textbf{Announcement Details View:} An asynchronous React Server Component responsible for rendering markdown content and managing authorized S3 image resolution.
    \item \textbf{Global Sidebar Injector:} Wraps the announcement feed in a Suspense boundary to surface it passively across all student-facing layouts.
\end{itemize}
...
\subsection{Module 5: Student Complaints}
\textbf{Purpose:}
Supports the formal submission of academic and administrative grievances by students. Complaints enter a multi-stage review pipeline, escalating progressively through the BA to the HOD.

\subsubsection{Key Functionalities:}
\begin{itemize}
    \item \textbf{Creation Interface:} Captures the complaint's title, descriptive details, categorical classification, and optional supporting images, offering real-time input validation.
    \item \textbf{Modification Constraints:} Restricts editing capabilities strictly to the initial \texttt{BA\_PENDING} state.
    \item \textbf{Deletion:} Supports individual and batch removal of complaints via synchronized confirmation modals.
    \item \textbf{Interactive Data Grid:} Presents a paginated table interface supporting complex sorting and multidimensional filtering (by status, category, date, and attachments).
    \item \textbf{Detail View:} Renders a detailed complaint card overlaying hierarchical metadata, administrative remarks, and attached evidence.
\end{itemize}

\textbf{Important Functions:}

\begin{itemize}
    \item \textbf{Schema Validation:} Employs structured Zod schemas to guarantee data integrity both at the client interface and within the receiving server action.
    \item \textbf{Server Action Pipeline:} Authenticates the active session, extracts the student's departmental affiliation from their user profile, and constructs the resulting database entity.
    \item \textbf{Data Fetcher:} Executes a dynamically scoped database query to populate the complaints table, strictly isolating the results to the authenticated student's authored records.
\end{itemize}
...
\textbf{Purpose:}
Admin can change any user's role, assign a department to professors, and appoint professors as BAs (each via a dedicated confirmation dialog).

\textbf{Key Functionality:}
\begin{itemize}
    \item \textbf{User Listing and Filtering:} Surfaces a paginated directory of platform users, executing minimal-latency server-side operations for role filtering, free-text search, and dynamic sorting over large datasets.
    \item \textbf{Role Management:} Supports the smooth elevation or modification of access tiers (e.g., granting Professor, Accountant, or Director privileges). The action automates the atomic removal of stale profile metadata and instantiates fresh structural records.
    \item \textbf{Departmental Allocation:} Enables the dynamic mapping of professors to respective departments while preserving organizational constraints. It enforces protective invariants, such as blocking the reassignment of active BAs to prevent cascading data inconsistencies.
    \item \textbf{Advisor Elevation Workflow:} Manages the transactional logic required to appoint a professor to the BA role, ensuring compliance with business rules (e.g., verifying the target department lacks a preexisting advisor).
\end{itemize}
...
\begin{itemize}
    ...
    \item makeProfessorBatchAdvisor: server action that atomically creates a BA record and links it to the corresponding professor and department.
\end{itemize}
...
\subsection{Module 7: HOD Announcements}

\textbf{Purpose:}
Provides department heads with a dedicated module to author, broadcast, and schedule administrative announcements tailored to their respective student bodies.

\textbf{Key Functionality:}
\begin{itemize}
    \item \textbf{Authoring Interface}: Supports the creation of rich announcements, supporting categorical tagging, timeline pinning, and image attachments. Payloads may be broadcast immediately or queued for delayed publication.
    \item \textbf{Asynchronous Scheduling}: Supports deferring announcements up to 48 hours into the future. The system relies on a reliable event-driven message queue (Inngest) to manage the background delivery state without manual intervention.
    \item \textbf{Data Retrieval}: Implements a solid paginated querying system with multi-dimensional filtering. The governing HOD can slice the dataset chronologically, temporally (Draft, Scheduled, Published), or by specialized parameters like attachment existence.
    \item \textbf{Bulk Data Operations}: Equips the management interface with batch processing capabilities, allowing administrators to modify or delete multiple entries simultaneously.
    \item \textbf{Detail Inspection}: Integrates a non-obtrusive drawer overlay to preview granular details, verifying rendering fidelity for attachments and specific temporal metadata markers before final publication.
\end{itemize}
...
This section structures the core business logic algorithms responsible for driving the platform's defining modules.
...
Algorithm 1 validates user registration inputs against the required format. Valid credentials are passed to the authentication layer to create a new user profile.
...
Algorithm 2 processes password reset requests. It validates the new password length and confirmation match. The credentials and token are transmitted to the authentication client to process the reset.
...
Algorithm 3 manages the creation of student complaints. It verifies the user session and validates permissions. After confirming the student's department, it validates the parameters against the schema and stores the complaint in the database.
...
\subsection{Algorithm 4: Submit Student LR}

\textbf{Input:} A session containing the $student\_id$ and LR data containing $date$, $reasonTitle$, $reasonDetails$, $subjectId$, and $imageKey$ \\
\textbf{Output:} An API response object indicating whether the LR was processed and approved \\

Algorithm 4 handles student LRs for specific lectures. It verifies user authorization, validates the required fields, and confirms the student's enrollment in the subject while checking for duplicate submissions.

\begin{algorithm}[H]
\caption{Submit LR Process}
\begin{algorithmic}[1]
...
\Function{SubmitLeaveRequest}{$input\_data, student\_id$}
    ...
    \If{\textbf{not} $has\_permission$}
        \State \Return $\{status: \text{"error"}, message: \text{"You are not allowed to create LRs"}\}$
    \EndIf
    ...
    \State \Comment{Verify the student is enrolled in this offering}
    ...
    \State \Comment{Prevent duplicate LRs for the same date and offering}
    ...
    \If{$existing\_request$ \textbf{is not} $null$}
        \State \Return $\{status: \text{"error"}, message: \text{"A LR for this subject and date already exists"}\}$
    \EndIf
    ...
    \If{$CreationResult.success$}
        \State \Return $\{status: \text{"success"}, message: \text{"LR created successfully."}\}$
    \Else
        \State \Return $\{status: \text{"error"}, message: \text{"Failed to send LR."}\}$
    \EndIf
...
\textit{Note: The BA and HOD review phases of the LR flow mirror the complaint review mechanisms (Algorithms 8–9). BA review can result in \texttt{REVIEW\_REQUESTED} (requesting more info from student), \texttt{HOD\_PENDING} (accepting to escalate), or \texttt{REJECTED}. When student resubmits from \texttt{REVIEW\_REQUESTED}, status resets to \texttt{PENDING}. HOD review then results in \texttt{APPROVED} or \texttt{REJECTED}. Algorithms 8 and 9 provide the generic implementation for both leave and complaint workflows.}
...
Algorithm 5 verifies HOD (HOD) permissions and validates announcement submissions. For scheduled announcements, it ensures the target date is within a 48-hour window.
...
Algorithm 6 describes the background job managed by Inngest. It validates the target date and schedules the function execution accordingly.
...
Algorithm 7 wraps the Nodemailer transport service. In development mode, it logs output to the console. In production, it formats an HTML template and uses the Gmail SMTP service to deliver emails.
...
\subsection{Algorithm 8: BA Complaint Review}
...
Algorithm 8 describes the BA (BA) review stage for student complaints. New submissions arrive as \textit{BA\_PENDING}. The BA can request additional information (\textit{BA\_REVIEW\_REQUESTED}), reject the complaint, or accept it for escalation (\textit{HOD\_PENDING}). The system records the decision, remarks, and timestamp.

\begin{algorithm}[htbp]
\caption{BA Decision Workflow}
\begin{algorithmic}[1]

\State \textbf{Input:} $session$, $complaint\_id$, $decision$, $remarks$
\State \textbf{Output:} $updated\_complaint$

\Function{ReviewComplaintAsBatchAdvisor}{$session, complaint\_id, decision, remarks$}

    \State \Comment{Authorize BA access}
    ...
    \State \Comment{Determine escalation, revision, or termination based on advisor decision}
    \If{$decision == \text{"accept"}$}
        \State $new\_status \gets \text{"HOD\_PENDING"}$
    \ElsIf{$decision == \text{"request\_info"}$}
        \State $new\_status \gets \text{"BA\_REVIEW\_REQUESTED"}$
    \Else
...
\subsection{Algorithm 9: HOD Complaint Review}

\textbf{Input:} A user session belonging to an HOD, the $complaint\_id$, and a review decision ($is\_accepted$, $is\_assigned$, along with optional $remarks$ and a $newDepartment$) \\
\textbf{Output:} An update to the specified complaint record culminating in final acceptance, rejection, or reassignment \\

Algorithm 9 outlines the HOD review process for escalated complaints. The HOD evaluates \textit{HOD\_PENDING} complaints to either reject, approve (\textit{HOD\_ACCEPTED}), or reassign them to another department (\textit{ASSIGNED}).

\begin{algorithm}[H]
\caption{HOD Decision Workflow}
\begin{algorithmic}[1]

\State \textbf{Input:} $complaint\_id$, $is\_accepted$, $is\_assigned$, $new\_dept$
\State \textbf{Output:} $updated\_complaint$

\Function{ReviewComplaintAsHOD}{$id, is\_accepted, is\_assigned$}
    ...
    \If{$is\_assigned$}
        ...
        \State $Database.Complaint.Update(id, \{status: \text{"ASSIGNED"}, dept: new\_dept\})$
        \State \Return $\text{"Complaint assigned."}$
    \EndIf
...
\subsection{Algorithm 10: Appoint BA}

\textbf{Input:} A user session belonging to an Admin, the target user's $userId$, and their specified $department$ \\
\textbf{Output:} An update to the specified user's roles and database profiles, elevating them to BA for that department \\

Algorithm 10 describes the BA appointment process. It ensures the candidate holds a Professor role in the target department and verifies the department does not already have an assigned advisor.

\begin{algorithm}[H]
\caption{BA Appointment Workflow}
\begin{algorithmic}[1]
...
\Function{AppointBatchAdvisor}{$userId, department$}
    ...
    \State \Comment{Step 4: Execute BA Appointment Checks}
    \If{$professor.batchAdvisor$ \textbf{is not} $null$}
        \State \Return $\{status: \text{"error"}, message: \text{"User is already a BA."}\}$
    \EndIf

    \State $existing\_ba \gets Database.BatchAdvisor.Find(\{ department \})$
    \If{$existing\_ba$ \textbf{is not} $null$}
        \State \Return $\{status: \text{"error"}, message: \text{"Department already has a BA."}\}$
    \EndIf
    ...
    \State \Return $\{status: \text{"success"}, message: \text{"Appointed as BA."}\}$
...
Algorithm 11 outlines the administrator workflow for creating academic offerings. It first checks for existing duplicates based on semester, year, subject, and department. Once the offering is created, it assigns the specified professor via an atomic upsert on the TeachingAssignment table.
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3cm}|p{8cm}|p{4cm}|}
        ...
        \hline
        \textbf{Inngest} &
        Provides serverless background jobs and scheduled task execution. It is used to handle asynchronous event-driven workflows, such as pausing execution to publish announcements at a scheduled date after a specific time limit. \newline
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{3cm}|p{8cm}|p{4cm}|}
        ...
        \hline
        \textbf{Better-Auth} &
        Acts as the primary complete authentication and authorization framework. It manages the full user lifecycle including sign-up, email/password login, email verification, password resets, and secure session management. It also integrates Role-Based Access Control (RBAC) to enforce permissions across different user roles (e.g., Admin, Student, Professor). \newline
        \textbf{Authentication:} Uses secure HTTP-only Next.js cookies and database-backed sessions via a Prisma PostgreSQL adapter. &
...
This section describes the user interfaces, outlining the purpose, layout, flow, and backend integrations for each main screen.
...
\textbf{Responsive design strategy} adopts a mobile-first approach using Tailwind's responsive utilities (breakpoints: sm 640px, md 768px, lg 1024px, xl 1280px, etc.). Components adapt fluidly via classes like sm md, ensuring smooth scaling across devices with adaptive patterns (e.g., collapsible navigation, drawers on mobile).
...
\textbf{Purpose:}
This screen enables new unauthenticated users to create an account in the system by providing their required details.
...
\textbf{Functional Flow:} Upon logging in, the student is redirected to the dashboard. The page automatically fetches the student's enrolled subjects. The subjects are displayed as cards in a responsive grid. The user can view their current attendance percentage for each subject. If attendance falls below the \text{80} threshold, the progress bar turns red and a warning message ("Below minimum requirement") is displayed. The user can click the "Attendance" link on any card to navigate to the detailed attendance page for that specific subject.
...
\begin{figure}[H]
  ...
  \caption{Student Dashboard Screen}
  \label{fig:student-dashboard}
\end{figure}

% -------------------------------------------------

\subsubsection{Screen 3: Student LR Screen}

\textbf{Purpose:}
This screen enables an authenticated student to submit a formal LR for a specific enrolled subject. The request is then forwarded to their respective Department BA for further review.

\textbf{Components:} \begin{itemize} \item \textbf{Text Content:} Page title (``New LR'') and informational text explaining the purpose of the form. \item \textbf{Navigation Link:} "View all LRs" link pointing to the student's leave history. \item \textbf{Form Inputs:} \begin{itemize} \item Subject Selection Dropdown \item Date Picker (Calendar Popover) \item Reason Title Input \item Description Textarea \item Image Evidence Uploader \end{itemize} \item \textbf{Submit Button:} Disables temporarily and shows a loading state ("Submitting...") during the form submission. \end{itemize}

\textbf{Functional Flow:} The user accesses the screen, which dynamically loads their currently enrolled subjects. They fill out the form by picking a subject, selecting a valid date, providing a title and detailed reason, and selectively attaching an image (e.g., a medical certificate). When the user clicks "Submit LR", the system attempts to send the request. If successful, the form is cleared, the page is refreshed to sync the latest state, and a success toast notification appears. If an error occurs, an error toast is displayed.
...
\begin{figure}[H]
  ...
  \caption{Student LR Screen}
  \label{fig:student-leave-request}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Student New Complaint Screen}
  \label{fig:student-complaint}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Professor Attendance Screen}
  \label{fig:professor-attendance}
\end{figure}

% -------------------------------------------------

\subsubsection{Screen 6: BA Complaints Screen}

\textbf{Purpose:}
This screen provides BAs with a centralized dashboard to view, manage, and process complaints submitted by students within their assigned batch.

\textbf{Components:}
\begin{itemize}
    \item \textbf{Header:} Page title ("BA Complaints") and a brief description.
...
\begin{figure}[H]
  ...
  \caption{BA Complaints Screen}
  \label{fig:ba-complaints}
\end{figure}
...
\begin{figure}[H]
  ...
  \label{fig:student-nav-flow}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{Figures/flow/acc-f.png}
  \caption{Accountant Navigation Flow}
  \label{fig:accountant-nav-flow}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Professor Navigation Flow}
  \label{fig:professor-nav-flow}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{Figures/flow/hod.png}
  \caption{HOD Navigation Flow}
  \label{fig:hod-nav-flow}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{Figures/flow/admin-f.png}
  \caption{Admin Navigation Flow}
  \label{fig:admin-nav-flow}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Parallel Independent Database Queries:} Uses \texttt{Promise.all()} to concurrently fetch lists and their total counts alongside required relations (e.g., in \texttt{get-complaints.ts}). This eliminates sequential waterfalls and reduces database round-trips, making data-heavy routes faster.
    ...
    \item \textbf{Non-Blocking UI Transitions:} Heavily uses \texttt{useTransition} for parameter updates via \texttt{nuqs} and form submissions. The interface remains fully interactive instead of freezing while Next.js fetches fresh Server Component payloads for sorting, filtering, or pagination.
...
\begin{itemize}
    ...
    \item \textbf{Data freshness:} Server Components fetch fresh data on parameter changes, avoiding the stale client cache issues that are common in pure client-side state management architectures.
    \item \textbf{Near-zero bundle impact:} Combining \texttt{nuqs} with built-in React hooks keeps the client JavaScript bundle small.
\end{itemize}
...
This section describes the platforms and services used for system deployment.
...
\hline
Security \& Bot Protection & Arcjet dashboard & Integrated security layer for rate limiting, bot detection, and malicious request blocking directly within the Next.js middleware and each actions. \\
\hline
\end{tabularx}
\end{table}
...
This chapter documented the implementation of the proposed student portal system, including development environments, system modules, algorithms, UI design, and deployment setups.

The development environment established the frameworks necessary for the system. Module and algorithm implementations covered authentication, LRs, complaint handling, and announcements. External services like Inngest and Better-Auth were integrated to support asynchronous tasks and role-based access.

The UI design section mapped out responsive interfaces matching user requirements. Finally, the deployment infrastructure described the serverless configuration on Vercel and Neon. This implementation provides a functioning system ready for testing.
