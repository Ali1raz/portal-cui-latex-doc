...
This chapter details the practical realization of the proposed system. It maps the architectural design to specific functional modules, algorithms, interface layouts, and deployment configurations.
...
This section outlines the software frameworks and hardware configurations employed during the system's development and deployment.
...
\textbf{Purpose:}
Allows students to formally submit leave requests for specific lectures. These requests are subsequently routed to the Batch Advisor (BA) and Head of Department (HOD) for review.
...
\begin{itemize}
\item Duplicate check: verifies whether a leave request already exists for the identical subject and date
...
\item Upon form submission, a server-side action re-validates the constraints:
  ...
  \item There must be no pre-existing pending or approved leave request for the same session
  \end{itemize}
\item Failed validations trigger a rejection and present localized error messages to the client. 
\item Upon success, the system instantiates a new leave request record with a \texttt{PENDING} status, assigning it to the corresponding Batch Advisor.
\end{itemize}
...
\textbf{Purpose:}
Supports the formal submission of academic and administrative grievances by students. Complaints enter a multi-stage review pipeline, escalating progressively through the Batch Advisor to the Head of Department.
...
\begin{itemize}
    ...
    \item \textbf{Departmental Allocation:} Enables the dynamic mapping of professors to respective departments while preserving organizational constraints. It enforces protective invariants, such as blocking the reassignment of active Batch Advisors to prevent cascading data inconsistencies.
    \item \textbf{Advisor Elevation Workflow:} Manages the transactional logic required to appoint a professor to the Batch Advisor role, ensuring compliance with business rules (e.g., verifying the target department lacks a preexisting advisor).
\end{itemize}
...
\subsection{Module 7: Head of Department Announcements}
...
The algorithm securely maps new user registrations by strictly validating payload inputs against the required formatting schema. Conforming credentials are incrementally passed to the authentication layer to establish a definitive profile record.
...
\textbf{Output:} An API response object indicating whether the leave request was processed and approved \\
...
\textit{Note: The algorithms for the Batch Advisor and Head of Department review phases of the Leave Request flow structurally mirror the general departmental review mechanisms (see Algorithms 8 and 9) and are omitted here for brevity.}
...
This algorithm maps the initial stage of a student's complaints flow. When a new submission arrives, its status defaults to \textit{BA\_PENDING} and is routed to the corresponding department. The BA retrieves these pending items and adds remarks. If rejected, the COMPL is halted pending student revision. If accepted, the status escalates to \textit{HOD\_PENDING}, awaiting final decision from the Head of Department. Both outcomes log the advisor's remarks and a precise review timestamp.
...
This algorithm maps the conclusive stage of a student's complaint's status. When a BA forwards a verified complaint, its status transitions to \textit{HOD\_PENDING}. The HOD assesses the COMPL. Rejection permanently halts the COMPL. Acceptance marks the complaint as successfully resolved (\textit{HOD\_ACCEPTED}). Importantly, if the complaint was incorrectly routed, the HOD can reassign the ticket to a new target department, resetting its state back to \textit{BA\_PENDING} within that new department's queue.
...
This section details the user interface architecture established for the platform. Each primary view is analyzed regarding its functional purpose, navigation pathways, validation boundaries, and explicit connections to the backend server architecture.
...
\begin{figure}[H]
  ...
  \caption{Student Leave Request Screen}
...
\begin{figure}[H]
  ...
  \caption{Batch Advisor Complaints Screen}
...
This section describes how we used the deployment platforms and architectural services in the project.
...
This chapter detailed the complete technical realization of the proposed student portal system. It bridged the gap between architectural theory and practical implementation, detailing the specific frameworks, data flows, and module integrations necessary to operationalize the platform.

The configuration of the development environment was outlined to establish a reproducible local setup. Each critical sub-system (such as authentication, student leave request handling, hierarchical complaint processing, and scheduled announcements) was documented conceptually and algorithmically. These modules were designed with strict decoupling principles to ensure reliable long-term maintainability.

Also, the chapter examined the role of specialized external APIs, such as Inngest for asynchronous job scheduling and Better-Auth for role-based session authorization. The user interface implementation verified the system's responsive layout strategy, mapping direct interactions to underlying server actions. 

The documented serverless deployment infrastructure, hosted across Vercel and Neon Serverless Postgres, confirms the platform's readiness for reliable production use. This thorough technical foundation sets the stage for the subsequent testing and evaluation phase.
