% File: abbreviations.tex

\newpage
\thispagestyle{plain}

\begin{center}
\textbf{\LARGE Abbreviations}
\end{center}

\vspace{1cm}

\begin{table}[h]
\centering
\renewcommand{\arraystretch}{1.4}
\begin{tabularx}{\textwidth}{|p{3cm}|X|}
\hline
\textbf{Abbreviation} & \textbf{Full Form} \\
\hline
\textbf{SRS} & Software Requirements Specification \\ \hline
\textbf{SDD} & Software Design Document \\ \hline
\textbf{API} & Application Programming Interface \\ \hline
\textbf{UI} & User Interface \\ \hline
\textbf{DB} & Database \\ \hline
\textbf{WCAG} & Web Content Accessibility Guidelines \\ \hline
\textbf{ARIA} & Accessible Rich Internet Applications \\ \hline
\textbf{BA} & Batch Advisor \\ \hline
\textbf{HOD} & Head of Department \\ \hline
\textbf{ANN} & Announcement \\ \hline
\textbf{COMPL} & Complaint \\ \hline
\textbf{UX} & User Experience \\ \hline
\textbf{DX} & Developer Experience \\ \hline
\end{tabularx}
\end{table}

\newpage
